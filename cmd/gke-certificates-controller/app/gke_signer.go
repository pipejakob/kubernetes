...
func (s *GKESigner) Sign(csr *certificates.CertificateSigningRequest) (*certificates.CertificateSigningRequest, error) {
	...
	if err := result.Error(); err != nil {
		// In the event of an error, try to unmarshal the response to
		// extract more information. If any part of the unmarshalling
		// fails, just report the generic result.Error() instead.
		// bytes := result.Raw()
...
func (s *GKESigner) webhookError(csr *certificates.CertificateSigningRequest, err error) error {
	...
	s.recorder.Eventf(csr, "Warning", "GKESigningError", "error while signing certificate: %v", err)
	return err
}

// signResultError represents the structured body of a failed call to GKE's
// SignCertificate.
...
// resultBodyError attempts to extract an error message out of an encoded
// response body.
